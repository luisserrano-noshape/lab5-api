...
public class TodoListController {
    ...
    public void shareList(Context ctx) {

        int listId = Integer.parseInt(ctx.pathParam("list-id"));

        ShareListRequest req = ctx.bodyAsClass(ShareListRequest.class);

        if (req.username == null || req.username.isEmpty()) {
            throw new BadRequestResponse("Username is required");
        }

        todoListService.shareList(listId, req.username);

        ctx.status(200).result("List shared successfully");
    }

    public static class ShareListRequest {
